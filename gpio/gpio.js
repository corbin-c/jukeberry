    ...
    this.config.leds.forEach((e) => {
      if (this.Gpio !== false) {
        ...
        this["led-" + e.name] = {
          ...
          blink: (t) => {
            if (
              typeof this.blinking[e.name] !== "undefined" &&
    ...
    this.config.buttons.forEach((e) => {
      if (this.Gpio !== false) {
        ...
        this.buttons[e.name].watch((error, value) => {
          ...
          if (value === 1) {
            if (
              typeof this.buttons[e.name].lastPush === "undefined" ||
              new Date().getTime() - this.buttons[e.name].lastPush > 300
            ) {
              this["btn-" + e.name].pushCallback();
              const combinations = this.combinations.filter((c) =>
                c.buttons.includes(e.name)
              );
              if (combinations.length > 0) {
                combinations.map(async (c) => {
                  ...
                  if (index == 0) {
                    ...
                    if (
                      c.lastButton.name == c.buttons[index - 1] &&
            ...
            if (
              typeof this.buttons[e.name].lastRelease === "undefined" ||
